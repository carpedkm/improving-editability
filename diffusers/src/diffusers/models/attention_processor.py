...
class AttnProcessor:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        print('ATTENTION PROCESSOR 1.0 CALLED')
...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # print('ATTENTION PROCESSOR 2.0 CALLED')
        if len(args) > 0 or kwargs.get("scale", None) is not None:
            deprecation_message = "The `scale` argument is deprecated and will be ignored. Please remove it, as passing it will raise an error in the future. `scale` should directly be passed while calling the underlying pipeline component i.e., via `cross_attention_kwargs`."
            deprecate("scale", "1.0.0", deprecation_message)
        ...
        #     # attention_mask = F.dropout(attention_mask, p=0.1, training=True)
        # else:
        #     query = attn.to_q(hidden_states)
        query = attn.to_q(hidden_states)
        if encoder_hidden_states is None:
        ...
        # # QUERING KEY AND VALUE as MASA-CTRL
        # if self.query_mode == 'cache':
        #     key = attn.to_k(encoder_hidden_states)
        #     value = attn.to_v(encoder_hidden_states)
        #     self.cached_queries.append((key, value))
        # elif self.query_mode == 'interpolate':
        #     vanilla_key, vanilla_value = self.cached_queries.pop(0)
        #     # key = attn.to_k(encoder_hidden_states, *args)
        #     # value = attn.to_v(encoder_hidden_states, *args)
        #     # key = (1 - self.vt) * key + self.vt * vanilla_key
        #     # value = (1 - self.vt) * value + self.vt * vanilla_value
        #     key = vanilla_key
        #     value = vanilla_value
        # else:
        #     key = attn.to_k(encoder_hidden_states)
